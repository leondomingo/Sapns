(function(a){function b(c){var d=a.extend(true,{name:"upl_"+Math.floor(Math.random()*999999),file_name:"",uploaded_file:"",upload_url:"{{tg.url('/docs/upload_file')}}",remove_url:"{{tg.url('/docs/remove_file')}}",repo:"",show_button:true,button_caption:"{{_('Upload')}}",auto_upload:true,qtip:{style:"ui-tooltip-red ui-tooltip-rounded",position:{my:"left center",at:"right center"}},onUpload:null,onDelete:null,onError:null,removeOnDelete:false,},c);
a.extend(true,this,d);this.file_size=0;this.uploaded=false}b.prototype.getFilename=function(){return this.file_name
...
c.setFilename("");c.setUploadedfile("");c.setUploaded(false)};if(g){a.ajax({url:c.remove_url,data:{file_name:e,id_repo:c.getRepo()},success:function(f){if(f.status){d()
}}})}else{d()}}};a.fn.sapnsUploader=function(f,d){if(f==undefined){f={}}if(typeof(f)==="object"){var g=new b(f);
var e=g.upload_url;var i=g.name;var h='<div id="file_name_'+i+'" class="sp-filename"><div style="float:left">...</div><button id="btn_delete_file_'+i+'">{{_("Delete")}}</button></div>';
...
}}}});this.find("#btn_upload_"+i).click(function(){g.upload()});this.data("sapnsUploader",g).addClass("sp-uploader")
