...
d(this,"onChange",null);d(this,"dialog",{});d(this.dialog,"width",950,this.dialog);
d(this.dialog,"height",550,this.dialog);d(this.dialog,"results",25,this.dialog)}a.prototype.setValue=function(g,f){var d=this;
...
var k='<input id="st_'+i.name+'" class="sp-select-text" type="text" readonly value=""';
...
var o=i.getValue();if(o!=""){var l=i.edit_url;var n='<form action="'+l+'" method="post" target="_blank"><input type="hidden" name="cls" value="'+m+'"><input type="hidden" name="id" value="'+o+'"><input type="hidden" name="came_from" value=""></form>';
b(n).appendTo("body").submit().remove()}}});var j=sprintf('{{_("Set a value for [%s]")}}',i.title);
var c='<button id="sb_'+i.name+'" class="sp-button sp-select-button"  title="'+j+'"  style="font-weight: bold;"';
if(i.read_only){c+=" disabled"}c+=">...</button>";this.append(c);this.find("#sb_"+i.name).click(function(){if(!i.isReadonly){var l=i.rc_title;
if(typeof(i.rc_title)=="function"){l=i.rc_title()}var m="dialog_"+i.name;b("#"+m).remove();
b('<div id="'+m+'"></div>').appendTo("body");b("#"+m).dialog({title:l,width:i.dialog.width,height:i.dialog.height,resizable:false,modal:true,close:function(){b("#"+m).remove()
},buttons:{"{{_('Ok')}}":function(){var n=b("#"+m+" .sapns_grid").sapnsGrid("getSelectedIds")[0];
i.setValue(n);i.setTitle();b("#"+m).dialog("destroy").remove()},"{{_('Cancel')}}":function(){b("#"+m).dialog("destroy").remove()
}}});i.search("")}});i.setTitle();var h='<button id="rb_'+i.name+'" class="sp-button sp-empty-button"';
if(i.read_only){h+=" disabled "}var j=sprintf("{{_('Remove value of [%s]')}}",i.title);
h+=' title="'+j+'"  style="font-weight: bold; color: red;">x</button>';this.append(h);
this.find("#rb_"+i.name).click(function(){if(!i.isReadonly){i.remove()}});this.data("sapnsSelector",i)
}else{if(typeof(g)=="string"){var i=this.data("sapnsSelector");if(g=="setValue"){i.setValue(f,d);
i.setTitle()}else{if(g=="getValue"){return i.value}else{if(g=="getTitle"){return i.title
}else{if(g=="getClass"){return i.rc}else{if(g=="setReadonly"){i.setReadonly(f)}else{if(g=="isReadonly"){return i.read_only
}}}}}}}}return this}})(jQuery);