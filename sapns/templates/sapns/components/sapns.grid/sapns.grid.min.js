...
var p=q.cols;if(typeof(p)=="function"){p=p()}var f=50;for(var x=0,u=p.length;x<u;
x++){var d=p[x];f+=(d.width+4)}var y='<div style="width: '+f+'px;">';if(!q.hide_check){y+='<div class="sp-grid-row"><div class="sp-col-title">#</div>'
...
if(c){y+='title="'+c.replace(/"/gi,"'")+'"'}else{y+='title="({{_("empty")}})"'}y+='clickable="true">';
y+=c;y+="</div>"}y+="</div>"}}else{var s=1;if(q.actions_inline){s=3}y+='<div class="sp-grid-row"><div class="sp-grid-cell sp-grid-noresults" style="width: 99%;" >{{_("No results")}}</div></div>'
...
}u+="</div>"}var p='<div class="sp-grid-parent" style="overflow: auto; clear: left; height: '+m.height+'px; background-color: transparent;"></div>';
