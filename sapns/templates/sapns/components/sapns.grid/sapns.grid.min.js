...
set(this,"default_",{});set(this.default_,"col_width",60,this.default_);set(this.default_,"col_align","center",this.default_);
...
}set(this,"exportable",true);var formats=[{id:"csv",title:"CSV",url:"/dashboard/tocsv"},{id:"excel",title:"Excel",url:"/dashboard/toxls"}];
...
}g_table+='<td class="sp-col-title" style="width: '+wd+'px;">'+col.title+"</td>"}g_table+="</tr>";
var data=self.data;if(typeof(data)=="function"){data=data()}var ld=data.length;if(ld>0){for(var i=0;
i<ld;i++){var row=data[i];g_table+='<tr class="sp-grid-row"><td title="'+(i+1)+'"><input class="sp-grid-rowid" type="checkbox" id_row="'+row[0]+'"></td>';
if(self.actions_inline){var _action_style='style="padding: 2px; margin-left: 5px; margin-right: 5px; border: 1px solid lightgray;"';
g_table+='<td style="font-size: 10px; width: 35px;"><a class="edit_inline" href="#" title="edit" '+_action_style+'>E</a><a class="delete_inline" href="#" title="delete" '+_action_style+'>D</a><a class="docs_inline" href="#" title="docs" '+_action_style+">D</a></td>"
...
}if(self.hide_id&&col.title=="id"){continue}var width="";if(wd>0){width=" width: "+wd+"px;"
}g_table+='<td><div class="sp-grid-cell" style="text-align: '+al+";"+width+'"';if(cell){g_table+='title="'+cell+'"'
}else{g_table+='title="({{_("empty")}})"'}g_table+='clickable="true">';g_table+=cell;
g_table+="</div></td>"}g_table+="</tr>"}}else{var n=1;if(self.actions_inline){n=3
}g_table+='<tr class="sp-grid-row" style="width: 100%;"><td class="sp-grid-cell sp-grid-noresults" colspan="'+(cols.length+n)+'">{{_("No results")}}</td></tr>'
...
}};SapnsGrid.prototype.search=function(q,force){var self=this;if(force==undefined){force=false
}if(q==undefined){q=self.q}if(self.q!=q||force){self.pag_n=1}self.q=q;var loading='<div style="padding: 10px; font-size: 15px; font-weight: bold; color: gray;">{{_("Loading")}}...</div>';
...
if(act.type==="new"){$("#search_box").append(a)}else{g_actions+=a}}else{g_actions+='<div style="float: left;"><button id="'+act.type.id+'" class="sp-button sp-grid-action"  require_id="'+req_id+'" >'+act.title+"</button></div>"
...
}g_actions+='<div id="grid-export_'+self.name+'" style="position: absolute; background-color: none; height: 25px; right: 0px;"><select id="select-export" class="sp-button sp-grid-action" style="height: 20px;"><option value="">({{_("Export")}})</option>'+options+"</select></div>"
...
if(self.actions.length==0&&!self.exportable){$("#"+self.name+" .actions").css("min-height","0px")
...
}else{$(this).data("_func")()}}})}}}}$("#"+self.name+" .sp-grid-cell").live("click",function(event){if($(this).attr("clickable")=="true"){var row_id=$(this).parent().parent().find(".sp-grid-rowid");
...
}else{row_id.attr("checked",true)}}});$("#"+self.name+" .standard_action").live("click",function(event){function form(action,target){var came_from="";
...
}f+="</form>";return f}var ch_attr=self.ch_attr;var target="";if(event.ctrlKey||event.metaKey){target="_blank"
...
$("#grid-dialog_"+self.name).html(delete_html);var title="";$.ajax({url:"/dashboard/title",type:"get",dataType:"json",data:{cls:cls,id:id},success:function(res){if(res.status){var title;
...
if(g.caption){g_content+='<div class="sp-grid-caption">'+g.caption+"</div>"}if(g.with_search){g_content+='<div><div id="search_box" style="float: left;"><input class="sp-search-txt" style="float: left;" name="q" type="text" value=""><button class="sp-button sp-search-btn" style="float: left;">{{_("Search...")}}</button></div>';
...
g.search(g.q)})}this.append(g_content+'<div class="actions" style="clear: left; position: relative; min-height: 20px;"></div>'+g_table+g_pager+"</div>");
