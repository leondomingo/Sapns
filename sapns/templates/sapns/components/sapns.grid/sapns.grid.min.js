...
var o=p.cols;if(typeof(o)=="function"){o=o()}var t=o.length,e=23+60;for(var x=0;x<t;
x++){var d=o[x];e+=d.width}var y='<div class="sp-grid" style="width: '+(e+10)+'px;">';
if(!p.hide_check){y+='<div class="sp-grid-row"><div class="sp-col-title" style="width: 23px; border-radius: 5px 0px 0px 0px;"><input class="sp-grid-select-all" type="checkbox"/></div>'
}if(p.actions_inline){y+='<div class="sp-col-title">*</div>'}for(var x=0,t=o.length;
x<t;x++){var d=o[x],m=d.width;if(!m){m=p.default_.col_width}if(p.hide_id&&d.title=="id"){continue
}var w="";if(x==t-1){w="border-radius: 0px 5px 0px 0px;"}if(p.hide_check&&x==0||p.hide_check&&p.hide_id&&x==1){w="border-radius: 5px 0px 0px 0px;"
}y+='<div class="sp-col-title" style="width: '+m+"px;"+w+'">'+d.title+"</div>"}y+="</div>";
var z=p.data;if(typeof(z)=="function"){z=z()}var q=z.length;if(q>0){for(var x=0;x<q;
x++){var f=z[x];y+='<div class="sp-grid-row">';if(!p.hide_check){var w="";if(x==q-1){w="border-radius: 0px 0px 0px 5px;"
}y+='<div class="sp-grid-cell" title="'+(x+1)+'" style="'+w+'"><input class="sp-grid-rowid" type="checkbox" id_row="'+f[0]+'"></div>'
}if(p.actions_inline){var g='style="padding: 2px; margin-left: 5px; margin-right: 5px; border: 1px solid lightgray;"';
y+='<div class="sp-grid-cell" style="font-size: 10px; width: 35px;"><a class="edit_inline" href="#" title="edit" '+g+'>E</a><a class="delete_inline" href="#" title="delete" '+g+'>D</a><a class="docs_inline" href="#" title="docs" '+g+">D</a></div>"
}for(var v=0,h=o.length;v<h;v++){var d=o[v],u=d.align;if(!u){u=p.default_.col_align
}var m=d.width;if(!m){m=p.default_.col_width}var c=f[v];if(!c){c=p.default_.empty_value
}if(p.hide_id&&d.title=="id"){continue}var s="";if(m>0){s=" width: "+m+"px;"}var w="";
if((p.hide_check&&x==q-1)&&(!p.hide_id&&v==0||p.hide_id&&v==1)){w="border-radius: 0px 0px 0px 5px;"
}if(x==q-1&&v==h-1){w="border-radius: 0px 0px 5px 0px;"}y+='<div class="sp-grid-cell" style="text-align: '+u+";"+s+w+'"';
if(c){y+='title="'+c.replace(/"/gi,"''")+'"'}else{y+='title="({{_("empty")}})"'}y+='clickable="true">'+c+"</div>"
}y+="</div>"}}else{var r=1;if(p.actions_inline){r=3}y+='<div class="sp-grid-row"><div class="sp-grid-cell sp-grid-noresults" style="width: '+e+'px; border-radius: 0px 0px 5px 5px;" >{{_("No results")}}</div></div>'
}y+="</div>";b("#"+p.name).find(".sp-grid-parent").html(y);if(p.select_first&&p.with_search&&p.q&&q<5&&q>0){b("#"+p.name+" .sp-grid-row input[type=checkbox]:first").attr("checked",true)
...
e.data=l.data;e.loadData();try{b(".sp-grid-cell").qtip({content:{text:true},position:{my:"top left",at:"bottom center"},style:"ui-tooltip-rounded"})
}catch(p){}if(f){f(l)}if(e.onLoad){e.onLoad(l)}}}},e.search_params);var c=JSON.stringify(d.data);
if(e.ajx_data){if(e.ajx_data===c&&!g){e.loadData()}else{b.ajax(d)}}e.ajx_data=c};
a.prototype.warningSelectedId=function(){var c=this;b("#grid-dialog_"+c.name).html("<p style='text-align: center; font-style: italic;'>{{_('You must select a row before click on this action')}}</p>").dialog({title:c.caption,resizable:false,height:155,width:450,modal:true,draggable:false,buttons:{"{{_('Close')}}":function(){b("#grid-dialog_"+c.name).dialog("close")
...
if(i.length>0){b(this).data("_func")(i[0],i)}else{b(this).data("_func")()}}})}}}}b("#"+g.name+" .sp-grid-cell").live("click",function(j){if(b(this).attr("clickable")=="true"){b("#"+g.name+" .sp-grid-select-all").attr("checked",false);
var i=b(this).parent().find(".sp-grid-rowid");b("#"+g.name+" .sp-grid-rowid").each(function(){var l=j.ctrlKey||j.metaKey;
if(b(this)!=i&&!g.multiselect&&!l){b(this).attr("checked",false)}});if(i.attr("checked")==true){i.attr("checked",false)
}else{i.attr("checked",true)}}});b("#"+g.name+" .standard_action").live("click",function(j){function n(x,y){var i="";
...
}}});b("#grid-dialog_"+e.name).dialog({width:650,height:210,resizable:false,modal:true,title:"{{_('Delete')}}",buttons:{"{{_('Ok')}}":function(){b.ajax({url:f,type:"get",dataType:"json",data:{cls:d,id_:j},success:function(m){if(m.status){e.search(e.q,true);
...
}}}}e+='<select class="sp-button sp-grid-rp">'+j+'<option value="10"'+n+'>10</option><option value="50"'+f+'>50</option><option value="100"'+s+'>100</option><option value="0"'+p+'>{{_("All")}}</option></select><button class="sp-button sp-grid-first-page" style="float: left;">|&lt;&lt;</button><button class="sp-button sp-grid-page-back" style="float: left;">&lt;&lt;</button><div><input class="sp-grid-current-page" type="text" style="text-align: center;" readonly></div><button class="sp-button sp-grid-page-forth" style="float: left;">&gt;&gt</button><button class="sp-button sp-grid-last-page" style="float: left;">&gt;&gt|</button>';
e+="</div>";b("#"+l.name+" .sp-grid-select-all").live("click",function(){var g=b(this).attr("checked");
b("#"+l.name+" .sp-grid-rowid").attr("checked",g)});b("#"+l.name+" .sp-grid-rp").live("change",function(){l.rp=b(this).val();
