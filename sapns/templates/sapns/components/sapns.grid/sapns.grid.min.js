...
if(h){j.push(new d({field:h[2],type:h[1]}))}}return j}function c(l){var i=this;function m(n,o,p,q){if(q===undefined){q=l
}if(q[o]===undefined){n[o]=p}else{n[o]=q[o]}return}m(i,"caption","");if(typeof(i.caption)=="function"){i.caption=i.caption()
}m(i,"name","grid_"+Math.floor(Math.random()*999999));m(i,"cls","");m(i,"with_search",true);
m(i,"search_params",{});m(i,"show_ids",false);m(i,"link","");m(i,"q","");m(i,"ch_attr","");
m(i,"parent_id","");m(i,"cols",[]);m(i,"data",{});m(i,"height",500);m(i,"url_base","");
m(i,"multiselect",false);m(i,"actions_inline",false);m(i,"hide_check",false);m(i,"hide_id",true);
m(i,"dblclick",null);m(i,"select_first",false);m(i,"onLoad",null);m(i,"default_",{});
m(i.default_,"col_width",60,this.default_);m(i.default_,"col_align","center",this.default_);
m(i.default_,"empty_value","",this.default_);m(i,"actions",null);if(typeof(i.actions)==="function"){i.actions=i.actions()
}m(i,"shift_enabled",false);m(i,"exportable",true);var h=[{id:"csv",title:"CSV",url:"{{tg.url('/dashboard/tocsv')}}"},{id:"excel",title:"Excel",url:"{{tg.url('/dashboard/toxls')}}"}];
m(i,"exportable_formats",h);m(i,"with_pager",true);m(i,"pag_n",1);m(i,"rp",10);m(i,"this_page",0);
m(i,"total_count",0);m(i,"total_pag",0);i.ajx_data="{}";i.styles=[];var j=i.q.split("$$");
i.q=j[0];i.filters=[];if(j.length>1){i.filters=f(j[1])}i.order=[];if(j.length>2){i.order=e(j[2])
}i._new_default={name:"new",url:'{{tg.url("/dashboard/new/")}}',require_id:"false",type:"new"};
...
i._docs_default={name:"docs",url:'{{tg.url("/dashboard/docs/")}}',require_id:"true",type:"docs"}
...
}if(O==G-1&&M==z-1){N="border-radius:0 0 5px 0;"}r+='<div class="sp-grid-cell sp-grid-cell-tip '+S+'"                         style="text-align:'+L+";"+J+N+'"';
...
var n=j.query();var o='<div style="padding:10px;font-size:15px; '+sprintf('font-weight:bold;color:gray;height:%(hg)dpx">{{_("Loading")}}...</div>',{hg:j.height-50});
...
g("#"+j.name+" .sp-grid-page-back").attr("title",p).attr("disabled",false)}else{g("#"+j.name+" .sp-grid-page-back").attr("disabled",true)
}if(u.curr_page<u.total_page){var t=sprintf("{{_('page %(p)d')}}",{p:u.curr_page+1});
g("#"+j.name+" .sp-grid-page-forth").attr("title",t).attr("disabled",false)}else{g("#"+j.name+" .sp-grid-page-forth").attr("disabled",true)
...
if(u){u()}}}var p=sprintf("#%s .sp-grid-cell",v.name);g(document).off("click",p).on("click",p,function(z){if(g(this).attr("clickable")=="true"){g("#"+v.name+" .sp-grid-select-all").prop("checked",false);
var y=g(this).parent().find(".sp-grid-rowid");g("#"+v.name+" .sp-grid-rowid").each(function(){var A=z.ctrlKey||z.metaKey;
if(g(this)!=y&&!v.multiselect&&!A){g(this).prop("checked",false)}});y.prop("checked",y.prop("checked")?false:true)
...
var D=A.parent().parent().find(".sp-grid-rowid").attr("id_row");q(D,[],y,B,z)}var i=sprintf("#%s .new_inline",v.name);
g(document).off("click",i).on("click",i,function(y){n(g(this),"new",y.ctrlKey||y.metaKey,y.shiftKey)
...
E+='<div class="sp-grid-filters" style="display:none"></div>';E+='<div class="sp-grid-edit-filter" style="display:none">                    <div style="float:left;height:40px;margin-right:5px">                        <div>Campo</div>                        <div><select class="sp-grid-filter-field"></select></div>                    </div>                    <div style="float:left;height:40px;margin-right:5px">                        <div>Operador</div>                        <div><select class="sp-grid-filter-operator">                            <option value="co">Contiene</option>                            <option value="eq">Igual</option>                            <option value="lt">Menor que</option>                            <option value="gt">Mayor que</option>                            <option value="let">Menor o igual que</option>                            <option value="get">Mayor o igual que</option>                            <option value="nco">No contiene</option>                            <option value="neq">Distinto</option>                        </select></div>                    </div>                    <div style="float:left;height:40px">                        <div>Valor</div>                        <input class="sp-grid-filter-value" type="text">                    </div></div>';
var p=".sp-grid-activate-filter, .sp-grid-deactivate-filter";g(document).off("click",p).on("click",p,function(){var q=g(this).parent().attr("filter_order");
var K=I.filters[q];if(K.active){g(this).removeClass("sp-grid-deactivate-filter").addClass("sp-grid-activate-filter");
g(this).text("Activar")}else{g(this).addClass("sp-grid-deactivate-filter").removeClass("sp-grid-activate-filter");
g(this).text("Desactivar")}g(this).addClass("sp-grid-filter-changed");K.toggle()});
var J=w+" .sp-search-btn";g(document).off("click",J).on("click",J,function(){function K(){var O="",L=I.filters.length;
if(L){for(var N=0;N<L;N++){var P=I.filters[N];var M="";if(P.active){M='<button class="sp-grid-deactivate-filter">Desactivar</button>'
}else{M='<button class="sp-grid-activate-filter">Activar</button>'}O+='<div class="sp-grid-row-filter" style="clear:left" filter_order="'+N+'">                                    <div style="float:left;margin-right:10px"><input class="sp-grid-check-filter" type="checkbox"></div>                                    <div class="sp-grid-filter-description">&lt;'+P.field+"&gt; "+P.operator_title().toLowerCase()+' <span style="font-style:italic">"'+P.value+'"</span></div>'+M+"</div>"
}}else{O='<p class="sp-grid-no-filters">No hay filtros</p>'}g(w+" .sp-grid-filters").html(O)
}K();function q(L,M){g("<div/>").html(g(w+" .sp-grid-edit-filter").html()).dialog({title:"Editar filtro",modal:true,resizable:false,width:700,height:150,open:function(){if(L){g(this).find(".sp-grid-filter-field").val(L.field);
g(this).find(".sp-grid-filter-operator").val(L.operator);g(this).find(".sp-grid-filter-value").val(L.value)
}},buttons:{Aceptar:function(){var N=this;if(!L){L=new b({field:g(this).find(".sp-grid-filter-field").val(),operator:g(this).find(".sp-grid-filter-operator").val(),value:g(this).find(".sp-grid-filter-value").val()});
...
g(this).dialog("close");if(M){M()}},Cancelar:function(){g(this).dialog("close")}}})
...
q(null,function(){L.dialog("close")})},buttons:{"Añadir":function(){var L=g(this);
q(null,function(){L.dialog("close")})},Editar:function(){var L=g(this).find(".sp-grid-row-filter input[type=checkbox]:visible:checked:first").parent().parent().attr("filter_order");
var M=g(this);q(I.filters[L],function(){M.dialog("close")});I.search(g(w+" .sp-search-txt").val(),true)
},Borrar:function(){var N=[];g(this).find(".sp-grid-row-filter input[type=checkbox]:visible:checked").each(function(){N.push(g(this).parent().parent().attr("filter_order")*1)
});N=N.reverse();for(var M=0,L=N.length;M<L;M++){I.filters.splice(N[M],1)}I.search(g(w+" .sp-search-txt").val(),true);
g(this).dialog("close")},"Borrar todos":function(){I.filters=[];I.search(g(w+" .sp-search-txt").val(),true);
...
I.search(g(w+" .sp-search-txt").val(),true)}g(this).dialog("close")}}})});var C=w+" .sp-search-txt";
g(document).off("keypress",C).on("keypress",C,function(q){if(q.which==13){I.search(g(this).val())
}});if(I.link){E+='<div style="padding-left:20px">                            <button id="link-shortcut" class="sp-button" style="float:left">{{_("Create a shortcut")}}</button>                            <div style="font-size:9px;margin-top:7px; width:100px;float:left">                            <a id="this-search" href="'+I.link+'"                             target="_blank">[{{_("this search")}}]</a>                            </div>                            <div id="link-shortcut-dialog" style="display:none">                            <p>{{_("Do you want to create a shortcut for this search?")}}</p>                            <label>{{_("Shortcut title")}}:</label>                            <input id="link-shortcut-title" type="text" value="({{_("title")}})">                            </div></div>'
