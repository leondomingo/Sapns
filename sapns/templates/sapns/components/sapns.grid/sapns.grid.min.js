...
var o=p.cols;if(typeof(o)=="function"){o=o()}var e=23;if(p.hide_check){e=-6}for(var x=0,t=o.length;
x<t;x++){var d=o[x];if(p.hide_id&&d.title=="id"){continue}e+=(d.width+5)}var y='<div class="sp-grid" style="width: '+(e+5)+'px;">';
if(!p.hide_check){y+='<div class="sp-grid-row"><div class="sp-col-title" style="width: 23px;"><input class="sp-grid-select-all" type="checkbox"/></div>'
...
}y+='<div class="sp-col-title" style="width: '+m+'px;">'+d.title+"</div>"}y+="</div>";
var z=p.data;if(typeof(z)=="function"){z=z()}var q=z.length;if(q>0){for(var x=0;x<q;
x++){var f=z[x];y+='<div class="sp-grid-row">';if(!p.hide_check){var w="";if(x==q-1){w="border-radius: 0 0 0 5px;"
}y+='<div class="sp-grid-cell" title="'+(x+1)+'" style="width: 23px;'+w+'"><input class="sp-grid-rowid" type="checkbox" id_row="'+f[0]+'"></div>'
...
if((p.hide_check&&x==q-1)&&(!p.hide_id&&v==0||p.hide_id&&v==1)){w="border-radius: 0 0 0 5px;"
}if(x==q-1&&v==h-1){w="border-radius: 0 0 5px 0;"}y+='<div class="sp-grid-cell" style="text-align: '+u+";"+s+w+'"';
if(c){y+='title="'+c.replace(/"/gi,"''")+'"'}else{y+='title="({{_("empty")}})"'}y+='clickable="true">'+c+"</div>"
}y+="</div>"}}else{var r=1;if(p.actions_inline){r=3}y+='<div class="sp-grid-row"><div class="sp-grid-cell sp-grid-noresults" title="{{_("No results")}}" style="width: '+e+'px;" >{{_("No results")}}</div></div>'
