...
if(!u.hide_check){C+=23}if(!u.hide_id){C+=60}var x=905;if(C<x){C=x}var N="";for(var V=0,T=z.length;
V<T;V++){var t=z[V];if(t.title!="id"){N+='<option value="'+t.title+'">'+t.title+"</option>"
}if(u.hide_id&&t.title==="id"){continue}A+=(t.width+5)}g("#"+u.name+" .sp-grid-filter-field").html(N);
var O='<div class="sp-grid" style="width:'+(A+200)+'px">';var I='<div class="sp-grid-row" style="width:'+(A+150)+'px">';
if(!u.hide_check){var M="";if(!u.allow_multiselect){M="disabled"}I+='<div class="sp-col-title" style="width:23px">                <input class="sp-grid-select-all" type="checkbox" '+M+"></div>"
...
}var p="#"+u.name;g(".sp-col-title-sortable").resizable({handles:"e",minWidth:u.resize.min_width,resize:function(o,n){var j=n.element.width(),X=n.element.attr("col_num"),l=g(".sp-grid-row").width(),i=g(".sp-grid-cell[col_num="+X+"]").width();
g(p+" .sp-grid-cell[col_num="+X+"]").width(j);g(p+" .sp-grid-row").width(l+(j-i))
},stop:function(j,i){var l=i.element.attr("col_num");if(u.resize.after){u.resize.after(l*1)
}else{g.ajax({url:"{{tg.url('/dashboard/save_col_width/')}}",data:{cls:u.cls,length:u.cols.length-1,col_num:l,width:i.element.width()},success:function(n){if(n.status){}}})
...
}else{g("#"+h.name+" .sp-search-btn").css("border-color","")}};g.fn.sapnsGrid=function(A,y,w){if(typeof(A)==="object"){var M=new c(A);
this.data("sapnsGrid",M);var x="#"+M.name;this.append(sprintf('<div id="grid-dialog_%(name)s" class="sp-grid-dialog" style="display:none"></div>',{name:M.name}));
var H="";H+=sprintf('<div class="sp-grid-container" id="%(name)s" cls="%(cls)s">',{name:M.name,cls:M.cls});
if(M.caption){H+=sprintf('<div class="sp-grid-caption">%(caption)s</div>',{caption:M.caption})
}if(M.with_search){H+='<div><div class="sp-grid-search-box">                    <input class="sp-search-txt" name="q" type="text" value="">                    <img class="inline_action sp-search-btn"                         src="{{tg.url("/images/sapns/icons/search.png")}}" title="{{_("Search...")}}">';
if(M.with_filters){var j="";for(var J=0,G=M.user_filters.length;J<G;J++){var N=M.user_filters[J],D="";
if(M.current_user_filter==N.name){D=" selected"}j+='<option value="'+N.name+'"'+D+">"+N.name+"</option>"
}H+='<img class="inline_action sp-grid-save-filter"                             src="{{tg.url("/images/sapns/icons/filters.png")}}" title="{{_("Manage filter")}}">                        <select class="sp-grid-action sp-button sp-grid-select-filter">'+j+"</select>"
}H+="</div>";H+='<div class="sp-grid-filters" style="display:none"></div>';H+='<div class="sp-grid-edit-filter" style="display:none">                    <div style="float:left;height:40px;margin-right:5px">                        <div class="sp-grid-filter-label">{{_("Field")}}</div>                        <div><select class="sp-grid-filter-field"></select></div>                    </div>                    <div style="float:left;height:40px;margin-right:5px">                        <div class="sp-grid-filter-label">{{_("Operator")}}</div>                        <div><select class="sp-grid-filter-operator">                            <option value="co">{{_("Contains")}}</option>                            <option value="eq">{{_("Equals to")}}</option>                            <option value="lt">{{_("Less than")}}</option>                            <option value="gt">{{_("Greater than")}}</option>                            <option value="let">{{_("Less than or equals to")}}</option>                            <option value="get">{{_("Greater than or equals to")}}</option>                            <option value="nco">{{_("Does not contain")}}</option>                            <option value="neq">{{_("Not equals to")}}</option>                        </select></div>                    </div>                    <div style="float:left;height:40px">                        <div class="sp-grid-filter-label">{{_("Value")}}</div>                        <input class="sp-grid-filter-value" type="text">                    </div></div>';
var r=".sp-grid-activate-filter, .sp-grid-deactivate-filter";g(document).off("keypress",".sp-grid-filter-value").on("keypress",".sp-grid-filter-value",function(i){if(i.which===13){g(this).parents(".ui-dialog").find(".ui-dialog-buttonset button:first").click()
...
var q=M.filters[l];if(q.active){g(this).parent().removeClass("active").addClass("inactive");
...
});var O=x+" .sp-search-btn";g(document).off("click",O).on("click",O,function(){function l(){var S="",q=M.filters.length;
if(q){for(var Q=0;Q<q;Q++){var T=M.filters[Q];var P="",R="active";if(T.active){P='<button>{{_("Deactivate")}}</button>'
}else{R="inactive";P='<button>{{_("Activate")}}</button>'}S+='<div class="sp-grid-row-filter '+R+'" style="clear:left" filter_order="'+Q+'">                                    <div style="float:left;margin-right:10px"><input class="sp-grid-check-filter" type="checkbox"></div>                                    <div class="sp-grid-filter-description"><span class="col">&lt;'+T.field+'&gt;</span>                                     <span class="operator">'+T.operator_title().toLowerCase()+'</span>                                    <span class="value">"'+T.value+'"</span></div>'+P+"</div>"
}}else{S='<p class="sp-grid-no-filters">{{_("There are no filters")}}</p>'}g(x+" .sp-grid-filters").html(S)
}l();function i(q,P){g("<div/>").html(g(x+" .sp-grid-edit-filter").html()).dialog({title:"{{_('Edit filter')}}",modal:true,resizable:false,width:700,height:150,open:function(){if(q){g(this).find(".sp-grid-filter-field").val(q.field);
g(this).find(".sp-grid-filter-operator").val(q.operator);g(this).find(".sp-grid-filter-value").val(q.value)
}},buttons:{"{{_('Ok')}}":function(){var Q=this;if(!q){q=new b({field:g(this).find(".sp-grid-filter-field").val(),operator:g(this).find(".sp-grid-filter-operator").val(),value:g(this).find(".sp-grid-filter-value").val()});
M.filters.push(q)}else{q.field=g(this).find(".sp-grid-filter-field").val();q.operator=g(this).find(".sp-grid-filter-operator").val();
q.value=g(this).find(".sp-grid-filter-value").val()}M.search(g(x+" .sp-search-txt").val(),true);
g(this).dialog("destroy").remove();if(P){P()}},"{{_('Cancel')}}":function(){g(this).dialog("destroy").remove()
}}})}g("<div/>").html(g(x+" .sp-grid-filters").html()).dialog({title:"Filtros",modal:true,resizable:false,width:600,height:300,closeOnEscape:false,open:function(){var q=g(this);
...
var P=g(this);i(M.filters[q],function(){P.dialog("destroy").remove()});M.search(g(x+" .sp-search-txt").val(),true)
},"{{_('Remove')}}":function(){var Q=[];g(this).find(".sp-grid-row-filter input[type=checkbox]:visible:checked").each(function(){Q.push(g(this).parent().parent().attr("filter_order")*1)
});Q=Q.reverse();for(var P=0,q=Q.length;P<q;P++){M.filters.splice(Q[P],1)}M.search(g(x+" .sp-search-txt").val(),true);
g(this).dialog("destroy").remove()},"{{_('Remove all')}}":function(){M.filters=[];
M.search(g(x+" .sp-search-txt").val(),true);g(this).dialog("destroy").remove()},"{{_('Close')}}":function(){if(g(".sp-grid-filter-changed").length){g(".sp-grid-filter-changed").removeClass("sp-grid-filter-changed");
M.search(g(x+" .sp-search-txt").val(),true)}g(this).dialog("destroy").remove()}}})
});var E=x+" .sp-search-txt";g(document).off("keypress",E).on("keypress",E,function(i){if(i.which===13){M.search(g(this).val())
}});var m=x+" .sp-grid-select-filter";g(document).off("change",m).on("change",m,function(){var S=g(this).val(),R="";
for(var P=0,q=M.user_filters.length;P<q;P++){var Q=M.user_filters[P];if(Q.name===S){M.setQuery(Q.query);
M.current_user_filter=S;M.search(M.query_().split("$$")[0],true);return}}});var I=x+" .sp-grid-save-filter";
g(document).off("click",I).on("click",I,function(){var q=M.query_(),l=g(m).val(),i=false,P='<div id="sp-grid-save-filter-dialog" style="display:none">                            <div class="sp-field-row">                                <div class="sp-field">                                    <div class="label">{{_("Filter name")}}</div>                                    <input id="sp-grid-filter-name" type="text" value="'+l+'">                                </div>                            </div>                        </div>';
g("#sp-grid-save-filter-dialog").remove();g(P).appendTo("body").dialog({title:"{{_('Save filter')}}",modal:true,resizable:false,width:400,height:"auto",buttons:{"{{_('Create or Modify')}}":function(){if(!i){i=true;
var Q=g("#sp-grid-filter-name").val().replace(/[^A-Za-z0-9_]/g,"_");g.ajax({url:"{{tg.url('/dashboard/save_user_filter/')}}",data:{cls:M.cls,filter_name:Q,query:q},success:function(T){if(T.status){var U=false;
for(var S=0,R=M.user_filters.length;S<R;S++){var V=M.user_filters[S];if(V.name===Q){V.query=q;
U=true}}if(!U){M.user_filters.push({name:Q,query:q});g(m).append('<option value="'+Q+'">'+Q+"</option>");
g(m).val(Q)}g("#sp-grid-save-filter-dialog").dialog("close")}else{if(T.msg){console.log(T.msg)
}i=false;alert("Error!")}},error:function(){i=false;alert("Error!")}})}},"{{_('Delete')}}":function(){if(!i){var Q=g("#sp-grid-filter-name").val();
i=true;g.ajax({url:"{{tg.url('/dashboard/delete_user_filter/')}}",data:{cls:M.cls,filter_name:Q},success:function(U){if(U.status){g(m).val("default").change();
g(m+" option[value="+Q+"]").remove();var T=[];for(var S=0,R=M.user_filters.length;
S<R;S++){var V=M.user_filters[S];if(V.name!==Q){T.push(V)}}M.user_filters=T;g("#sp-grid-save-filter-dialog").dialog("close")
...
}}}})});H+="</div>"}var L='<div class="sp-grid-parent" style="overflow:auto;clear:left;height:'+(M.height+5)+'px;background-color:transparent"></div>';
var u="";if(M.with_pager){u+='<div class="sp-grid-pager">                    <div class="sp-grid-pager-desc"></div>';
var F="";for(var J=0,G=M.pager_options.length;J<G;J++){var z=M.pager_options[J];var D="";
if(z.sel){D=" selected "}F+='<option value="'+z.val+'"'+D+">"+z.desc+"</option>"}u+='<div style="float:left;clear:right;height:25px;margin-top:2px"><select class="sp-button sp-grid-rp">'+F+'</select><button class="sp-button sp-grid-first-page" style="float:left">|&lt;&lt;</button><button class="sp-button sp-grid-page-back" style="float:left">&lt;&lt;</button><input class="sp-grid-current-page" type="text" style="text-align:center;font-size:11px;margin-top:3px" readonly><button class="sp-button sp-grid-page-forth" style="float:left">&gt;&gt</button>';
u+="</div>";var v=x+" .sp-grid-select-all",s=x+" .sp-grid-rp",p=x+" .sp-grid-first-page",o=x+" .sp-grid-page-back",n=x+" .sp-grid-page-forth",h=x+" .sp-grid-last-page";
g(document).off("click",v).on("click",v,function(){var i=g(this).prop("checked");
M.selectAll(i)});g(document).off("change",s).on("change",s,function(){M.rp=g(this).val();
M.pag_n=1;M.search(M.q)});g(document).off("click",p).on("click",p,function(){M.pag_n=1;
M.search(M.q)});g(document).off("click",o).on("click",o,function(){if(M.pag_n>1){M.pag_n-=1;
M.search(M.q)}});g(document).off("click",n).on("click",n,function(){if(M.pag_n<M.total_pag){M.pag_n*=1;
M.pag_n+=1;M.search(M.q)}});g(document).off("click",h).on("click",h,function(){M.pag_n=M.total_pag;
M.search(M.q)})}var K=x+" .sp-col-title-sortable";g(document).off("click",K).on("click",K,function(S){var U=g(this).attr("col_title");
var P=S.shiftKey;var T=S.ctrlKey||S.metaKey;if(g(this).hasClass("sp-grid-ordered-col")){var R;
if(g(this).attr("order_type")=="asc"){R="desc"}else{R="asc"}if(P){for(var Q=0,q=M.order.length;
Q<q;Q++){if(M.order[Q].field===U){break}}M.order[Q].type=R}else{if(!T){if(M.order.length>1){R="asc"
}var V=new d({field:U,type:R});M.order=[V]}else{for(var Q=0,q=M.order.length;Q<q;
Q++){if(M.order[Q].field===U){break}}M.order.splice(Q,1);g(this).removeClass("sp-grid-ordered-col")
}}}else{var V=new d({field:U,type:"asc"});if(P){M.order.push(V)}else{M.order=[V]}}M.search(undefined,true)
});var t='<div class="sp-grid-button-actions" style="display:none"></div>';this.append(H+t+L+u+"</div>");
M.loadActions(function(){g(x+" .sp-search-txt").val(M.q);M.search(M.q)})}else{if(typeof(A)=="string"){var C=this.data("sapnsGrid");
var x="#"+C.name;if(A=="loadData"){C.loadData()}else{if(A=="search"){var B="";if(typeof(y)=="boolean"){B=C.q
}else{if(y){B=y}}if(C.with_search){g(x+" .sp-search-txt").val(B)}C.search(B,true,w)
}else{if(A=="getSelectedIds"){return C.getSelectedIds()}else{if(A=="setRp"){C.rp=y;
g(x+" .sp-grid-rp [value="+y+"]").prop("selected",true)}else{if(A=="selectAll"){C.selectAll()
}else{if(A==="getQuery"){return C.query_()}else{if(A==="setQuery"){C.setQuery(y)}}}}}}}}}return this
}})(jQuery);