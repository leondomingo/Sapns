...
}if(typeof(n.actions)==="function"){n.actions=n.actions()}var h=[{id:"excel",title:"Excel",url:"{{tg.url('/dashboard/to_xls/')}}",func_name:"to_xls",export_url:"{{tg.url('/dashboard/to_xls_/')}}",modal:true},{id:"csv",title:"CSV",url:"{{tg.url('/dashboard/to_csv/')}}"}];
...
}var A=true;if(U.refresh!==undefined){A=U.refresh}if(U.extra_params!==undefined){for(ep in U.extra_params){Q[ep]=U.extra_params[ep]
}}var E=r.name+"_dialog";g("#"+E).remove();g('<div id="'+E+'" style="display:none"></div>').appendTo("body");
var L=[];if(U.buttons===undefined){U.buttons=[{button_title:B,callback:U.callback,refresh:A}]
...
g("#sp-grid-export-dialog").html(K.content).dialog({title:I.title,modal:true,resizable:false,width:I.width||700,height:I.height||"auto",buttons:{"{{_('Export')}}":function(){if(!J){J=true;
window[I.func_name](function(){J=false},function(){J=false})}},"{{_('Close')}}":function(){if(!J){g("#sp-grid-export-dialog").dialog("close")
