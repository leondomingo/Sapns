...
if(h){j.push(new d({field:h[2],type:h[1]}))}}return j}function c(m){var i=this;if(typeof(m.caption)==="function"){m.caption=m.caption()
}if(typeof(m.actions)==="function"){m.actions=m.actions()}var h=[{id:"csv",title:"CSV",url:"{{tg.url('/dashboard/tocsv/')}}"},{id:"excel",title:"Excel",url:"{{tg.url('/dashboard/toxls/')}}"}];
var l=m.pager_options;if(!l){l=[{val:10,desc:"10",sel:true},{val:50,desc:"50",sel:false},{val:100,desc:"100",sel:false},{val:0,desc:"{{_('All')}}",sel:false}]
}var n=g.extend(true,{caption:"",name:"sapns_grid_"+Math.floor(Math.random()*999999),cls:"",with_search:true,search_params:{},show_ids:false,link:"",q:"",ch_attr:"",parent_id:"",cols:[],data:{},height:500,url_base:"",multiselect:false,allow_multiselect:true,actions:null,actions_inline:false,hide_check:false,hide_id:true,dblclick:null,select_first:false,shift_enabled:false,exportable:true,exportable_formats:h,with_pager:true,pag_n:1,rp:10,onLoad:null,default_:{col_width:60,col_align:"center",empty_value:""}},m);
g.extend(true,i,n);i.pager_options=l;i.this_page=0;i.total_count=0;i.total_pag=0;
i.ajx_data=JSON.stringify({});i.styles=[];var j=i.q.split("$$");i.q=j[0];i.filters=[];
if(j.length>1){i.filters=f(j[1])}i.order=[];if(j.length>2){i.order=e(j[2])}i._new_default={name:"new",url:'{{tg.url("/dashboard/new/")}}',require_id:"false",type:"new"};
...
if(l.attr("id_row")===j){i=l.parent();return}});return i};c.prototype.loadData=function(v){var F=this;
var D=F.cols;if(typeof(D)==="function"){D=D()}var x=23;if(F.hide_check){x=-6}var t=D.length*42;
if(!F.hide_check){t+=23}if(!F.hide_id){t+=60}var m=905;if(t<m){t=m}var B="";for(var P=0,L=D.length;
P<L;P++){var w=D[P];if(w.title!="id"){B+='<option value="'+w.title+'">'+w.title+"</option>"
}if(F.hide_id&&w.title==="id"){continue}x+=(w.width+5)}g("#"+F.name+" .sp-grid-filter-field").html(B);
var Q='<div class="sp-grid" style="width:'+(x+200)+'px">';var p='<div class="sp-grid-row" style="width:'+(x+150)+'px">';
if(!F.hide_check){var u="";if(!F.allow_multiselect){u="disabled"}p+='<div class="sp-col-title" style="width:23px">                <input class="sp-grid-select-all" type="checkbox" '+u+"></div>"
}if(F.actions_inline){p+='<div class="sp-col-title" style="%(actions_wd)s">*</div>'
}var G={};for(var P=0,L=F.order.length;P<L;P++){var I=F.order[P];I.index=P+1;G[F.order[P].field]=I
}for(var P=0,L=D.length;P<L;P++){var w=D[P];var C=w.width;if(!C){C=F.default_.col_width
}if(F.hide_id&&w.title==="id"){continue}var s="";var U="";var E="";if(G[w.title]){var I=G[w.title];
s="sp-grid-ordered-col";U=' order_type="'+I.type+'"';var z;if(I.type==="asc"){z="+"
}else{z="-"}E=" ("+I.index+z+")"}p+='<div class="sp-col-title sp-col-title-sortable '+s+'"'+U+' style="width:'+C+'px" col_title="'+w.title+'">'+w.title+E+"</div>"
}p+="</div>";var V=F.data;if(typeof(V)==="function"){V=V()}var H=V.length;if(H>0){for(var P=0;
P<H;P++){var y=V[P];var T="";if(F.styles.length&&F.styles.length>P){T=F.styles[P]
}var r='<div class="sp-grid-row" style="width:'+(x+150)+'px">';if(!F.hide_check){var O="";
if(P===H-1){O="border-radius:0 0 0 5px"}r+='<div class="sp-grid-cell" title="'+y[0]+'" style="width:23px;'+O+'"><input class="sp-grid-rowid" type="checkbox" id_row="'+y[0]+'"></div>'
}if(F.actions_inline){var h="width:107px";if(!F.nonstd){h="width:82px"}var S='<div class="sp-grid-cell" style="%(actions_wd)s">                        <img class="inline_action edit_inline" title="{{_("Edit")}}"                             src="{{tg.url("/images/sapns/icons/edit.png")}}">                        <img class="inline_action delete_inline" title="{{_("Delete")}}"                             src="{{tg.url("/images/sapns/icons/delete.png")}}">                        <img class="inline_action docs_inline" title="{{_("Docs")}}"                             src="{{tg.url("/images/sapns/icons/docs.png")}}">';
r+=sprintf(S,{actions_wd:h})+F.nonstd+"</div>"}if(P===0){Q+=sprintf(p,{actions_wd:h})
}for(var N=0,A=D.length;N<A;N++){var w=D[N];var M=w.align;if(!M){M=F.default_.col_align
}var C=w.width;if(!C){C=F.default_.col_width}var q=y[N];if(!q){q=F.default_.empty_value
}if(F.hide_id&&w.title=="id"){continue}var K="";if(C>0){K=" width:"+C+"px;"}var O="";
if((F.hide_check&&P==H-1)&&(!F.hide_id&&N==0||F.hide_id&&N==1)){O="border-radius:0 0 0 5px;"
}if(P==H-1&&N==A-1){O="border-radius:0 0 5px 0;"}r+='<div class="sp-grid-cell sp-grid-cell-tip clickable '+T+'"                         style="text-align:'+M+";"+K+O+'"';
if(q){r+=sprintf('title="%s"',q.replace(/"/gi,"''"))}else{r+='title="({{_("empty")}})"'
}r+=">"+q+"</div>"}r+="</div>";Q+=r}}else{var J=1;var R=x;if(F.actions_inline){J=3;
R+=25}Q+=sprintf(p,{actions_wd:h})+'<div class="sp-grid-row"><div class="sp-grid-cell sp-grid-noresults" title="{{_("No results")}}" style="width:'+R+'px" >{{_("No results")}}</div></div>'
}Q+="</div>";g("#"+F.name).find(".sp-grid-parent").html(Q);g(".sp-col-title-sortable").disableSelection();
if(F.select_first&&F.with_search&&F.q&&H<5&&H>0){g("#"+F.name+" .sp-grid-rowid:first").prop("checked",true)
}if(v){v()}};c.prototype.query=function(){var h=this;var j=h.q;var i=g.map(h.filters,function(l){if(l.active){return l
...
function h(F,G){var D=r.q+"$$";for(var C=0,A=r.filters.length;C<A;C++){if(C>0){D+=", "
}D+=r.filters[C].stringify()}D+="$$";for(var C=0,A=r.order.length;C<A;C++){if(C>0){D+=", "
}D+=r.order[C].stringify()}D=encodeURI(D).replace("-","%2D","g").replace('"',"%22","g").replace("+","%2B","g").replace("#","%23","g");
var B="";if(G!="_blank"){var B=r.url_base+"?q="+D+"&rp="+r.rp+"&pag_n="+r.pag_n+"&ch_attr="+r.ch_attr+"&parent_id="+r.parent_id
}var E='<form type="post" action="'+F+'" target="'+G+'"><input type="hidden" name="came_from" value="'+B+'">';
if(r.ch_attr){E+='<input type="hidden" name="_'+r.ch_attr+'" value="'+r.parent_id+'">'
}E+="</form>";return E}function w(C,B,K,A,D){if(typeof(K.type)==="string"){if(D&&r.shift_enabled){if(K.type==="new"){K=r._new_default
}else{if(K.type==="edit"){K=r._edit_default}else{if(K.type==="delete"){K=r._delete_default
}else{if(K.type==="docs"){K=r._docs_default}}}}}var L=K.url;if(L[L.length-1]!="/"){L+="/"
}if(!K.data){var J="";if(A){J="_blank"}if(K.type==="process"){if(K.require_id){if(C){L+=C
}else{r.warningSelectedId();return}}else{if(C){L+=C}}}else{if(K.type==="new"){L+=sprintf("%s/",r.cls)
}else{if(K.type==="delete"){r.std_delete(B,K.url);return}else{if(K.require_id){if(C){L+=sprintf("%s/%s",r.cls,C)
}else{r.warningSelectedId();return}}else{if(C){L+=sprintf("%s/%s",r.cls,C)}else{L+=sprintf("%s/",r.cls)
}}}}}g(h(L,J)).appendTo("body").submit().remove()}else{var I=JSON.parse(K.data),F=false;
var G={};G[I.param_name]=C;var H=r.name+"_dialog";g("#"+H).remove();g('<div id="'+H+'" style="display:none"></div>').appendTo("body");
g.ajax({url:L,data:G,success:function(M){g("#"+H).html(M).dialog({title:K.title,modal:true,resizable:false,width:I.width,height:I.height,closeOnEscape:false,buttons:{"{{_('Ok')}}":function(){if(!F){F=true;
window[I.callback](function(){r.search(r.q,true);g("#"+H).dialog("close")},function(){F=false
})}},"{{_('Cancel')}}":function(){if(!F){g("#"+H).dialog("close")}}}})}})}}else{var E=B;
if(K.require_id){if(E.length>0){K.type.f(E[0],E)}else{r.warningSelectedId();return
}}else{if(E.length>0){K.type.f(E[0],E)}else{K.type.f()}}}}function p(C,E,D,B){var A=r.getAction(E);
...
if(A){p(g(this),A)}})}else{var x=sprintf("#%s .sp-grid-button-action",r.name);g(document).off("click",x).on("click",x,function(B){var A=r.getAction(g(this).attr("id"));
var C=r.getSelectedIds();w(C[0],C,A,B.ctrlKey||B.metaKey,B.shiftKey)})}var u=sprintf("#%s .export select",r.name);
g(document).off("change","export_sel").on("change",u,function(){var C=g(u).val();
if(C==""){return}var A=r.exportable_formats;var H="";if(typeof(r.exportable)==="object"){A=r.exportable.formats;
if(r.exportable.data){for(k in r.exportable.data){var D=r.exportable.data[k];if(typeof(D)==="function"){D=D()
}H+='<input type="hidden" name="'+k+'" value="'+D+'">'}}}var F="";for(var G=0,B=A.length;
G<B;G++){if(A[G].id==C){F=A[G].url;break}}var E='<form action="'+F+'" method="get">                <input type="hidden" name="cls" value="'+r.cls+'">                <input type="hidden" name="q" value="'+r.query()+'">                <input type="hidden" name="ch_attr" value="'+r.ch_attr+'">                <input type="hidden" name="parent_id" value="'+r.parent_id+'">'+H+"</form>";
...
}else{g("#"+h.name+" .sp-search-btn").css("border-color","")}};g.fn.sapnsGrid=function(y,x,v){if(typeof(y)=="object"){var K=new c(y);
this.data("sapnsGrid",K);var w="#"+K.name;this.append(sprintf('<div id="grid-dialog_%(name)s" class="sp-grid-dialog" style="display:none"></div>',{name:K.name}));
var G="";G+=sprintf('<div class="sp-grid-container" id="%(name)s" cls="%(cls)s">',{name:K.name,cls:K.cls});
if(K.caption){G+=sprintf('<div class="sp-grid-caption">%(caption)s</div>',{caption:K.caption})
}if(K.with_search){G+='<div><div class="sp-grid-search-box"><input class="sp-search-txt" style="float:left" name="q" type="text" value=""><img class="inline_action sp-search-btn" src="{{tg.url("/images/sapns/icons/search.png")}}" titlee="{{_("Search...")}}"></div>';
G+='<div class="sp-grid-filters" style="display:none"></div>';G+='<div class="sp-grid-edit-filter" style="display:none">                    <div style="float:left;height:40px;margin-right:5px">                        <div class="sp-grid-filter-label">{{_("Field")}}</div>                        <div><select class="sp-grid-filter-field"></select></div>                    </div>                    <div style="float:left;height:40px;margin-right:5px">                        <div class="sp-grid-filter-label">{{_("Operator")}}</div>                        <div><select class="sp-grid-filter-operator">                            <option value="co">{{_("Contains")}}</option>                            <option value="eq">{{_("Equals to")}}</option>                            <option value="lt">{{_("Less than")}}</option>                            <option value="gt">{{_("Greater than")}}</option>                            <option value="let">{{_("Less than or equals to")}}</option>                            <option value="get">{{_("Greater than or equals to")}}</option>                            <option value="nco">{{_("Does not contain")}}</option>                            <option value="neq">{{_("Not equals to")}}</option>                        </select></div>                    </div>                    <div style="float:left;height:40px">                        <div class="sp-grid-filter-label">{{_("Value")}}</div>                        <input class="sp-grid-filter-value" type="text">                    </div></div>';
var p=".sp-grid-activate-filter, .sp-grid-deactivate-filter";g(document).off("keypress",".sp-grid-filter-value").on("keypress",".sp-grid-filter-value",function(i){if(i.which===13){g(this).parents(".ui-dialog").find(".ui-dialog-buttonset button:first").click()
}});g(document).off("click",".sp-grid-row-filter button").on("click",".sp-grid-row-filter button",function(){var l=g(this).parent().attr("filter_order");
var q=K.filters[l];if(q.active){g(this).parent().removeClass("active").addClass("inactive");
...
});var L=w+" .sp-search-btn";g(document).off("click",L).on("click",L,function(){function l(){var P="",q=K.filters.length;
if(q){for(var N=0;N<q;N++){var Q=K.filters[N];var M="",O="active";if(Q.active){M='<button>{{_("Deactivate")}}</button>'
...
}l();function i(q,M){g("<div/>").html(g(w+" .sp-grid-edit-filter").html()).dialog({title:"{{_('Edit filter')}}",modal:true,resizable:false,width:700,height:150,open:function(){if(q){g(this).find(".sp-grid-filter-field").val(q.field);
g(this).find(".sp-grid-filter-operator").val(q.operator);g(this).find(".sp-grid-filter-value").val(q.value)
}},buttons:{"{{_('Ok')}}":function(){var N=this;if(!q){q=new b({field:g(this).find(".sp-grid-filter-field").val(),operator:g(this).find(".sp-grid-filter-operator").val(),value:g(this).find(".sp-grid-filter-value").val()});
K.filters.push(q)}else{q.field=g(this).find(".sp-grid-filter-field").val();q.operator=g(this).find(".sp-grid-filter-operator").val();
q.value=g(this).find(".sp-grid-filter-value").val()}K.search(g(w+" .sp-search-txt").val(),true);
g(this).dialog("destroy").remove();if(M){M()}},"{{_('Cancel')}}":function(){g(this).dialog("destroy").remove()
}}})}g("<div/>").html(g(w+" .sp-grid-filters").html()).dialog({title:"Filtros",modal:true,resizable:false,width:600,height:300,closeOnEscape:false,open:function(){var q=g(this);
...
var M=g(this);i(K.filters[q],function(){M.dialog("destroy").remove()});K.search(g(w+" .sp-search-txt").val(),true)
...
});N=N.reverse();for(var M=0,q=N.length;M<q;M++){K.filters.splice(N[M],1)}K.search(g(w+" .sp-search-txt").val(),true);
g(this).dialog("destroy").remove()},"{{_('Remove all')}}":function(){K.filters=[];
K.search(g(w+" .sp-search-txt").val(),true);g(this).dialog("destroy").remove()},"{{_('Close')}}":function(){if(g(".sp-grid-filter-changed").length){g(".sp-grid-filter-changed").removeClass("sp-grid-filter-changed");
K.search(g(w+" .sp-search-txt").val(),true)}g(this).dialog("destroy").remove()}}})
});var D=w+" .sp-search-txt";g(document).off("keypress",D).on("keypress",D,function(i){if(i.which===13){K.search(g(this).val())
}});G+="</div>"}var J='<div class="sp-grid-parent" style="overflow:auto;clear:left;height:'+(K.height+5)+'px;background-color:transparent"></div>';
var t="";if(K.with_pager){t+='<div class="sp-grid-pager">                    <div class="sp-grid-pager-desc"></div>';
var F="",j=[];for(var H=0,E=K.pager_options.length;H<E;H++){var z=K.pager_options[H];
var C="";if(z.sel){C=" selected "}F+='<option value="'+z.val+'"'+C+">"+z.desc+"</option>";
j.push(z.val)}if(g.inArray(K.rp,j)===-1){F+='<option value="'+K.rp+'" selected>'+K.rp+"</option>"
}t+='<div style="float:left;clear:right;height:25px;margin-top:2px"><select class="sp-button sp-grid-rp">'+F+'</select><button class="sp-button sp-grid-first-page" style="float:left">|&lt;&lt;</button><button class="sp-button sp-grid-page-back" style="float:left">&lt;&lt;</button><input class="sp-grid-current-page" type="text" style="text-align:center;font-size:11px;margin-top:3px" readonly><button class="sp-button sp-grid-page-forth" style="float:left">&gt;&gt</button>';
t+="</div>";var u=w+" .sp-grid-select-all",r=w+" .sp-grid-rp",o=w+" .sp-grid-first-page",n=w+" .sp-grid-page-back",m=w+" .sp-grid-page-forth",h=w+" .sp-grid-last-page";
g(document).off("click",u).on("click",u,function(){var i=g(this).prop("checked");
K.selectAll(i)});g(document).off("change",r).on("change",r,function(){K.rp=g(this).val();
K.pag_n=1;K.search(K.q)});g(document).off("click",o).on("click",o,function(){K.pag_n=1;
K.search(K.q)});g(document).off("click",n).on("click",n,function(){if(K.pag_n>1){K.pag_n-=1;
K.search(K.q)}});g(document).off("click",m).on("click",m,function(){if(K.pag_n<K.total_pag){K.pag_n*=1;
K.pag_n+=1;K.search(K.q)}});g(document).off("click",h).on("click",h,function(){K.pag_n=K.total_pag;
K.search(K.q)})}var I=w+" .sp-col-title-sortable";g(document).off("click",I).on("click",I,function(P){var R=g(this).attr("col_title");
var M=P.shiftKey;var Q=P.ctrlKey||P.metaKey;if(g(this).hasClass("sp-grid-ordered-col")){var O;
if(g(this).attr("order_type")=="asc"){O="desc"}else{O="asc"}if(M){for(var N=0,q=K.order.length;
N<q;N++){if(K.order[N].field===R){break}}K.order[N].type=O}else{if(!Q){if(K.order.length>1){O="asc"
}var S=new d({field:R,type:O});K.order=[S]}else{for(var N=0,q=K.order.length;N<q;
N++){if(K.order[N].field===R){break}}K.order.splice(N,1);g(this).removeClass("sp-grid-ordered-col")
}}}else{var S=new d({field:R,type:"asc"});if(M){K.order.push(S)}else{K.order=[S]}}K.search(undefined,true)
});var s='<div class="sp-grid-button-actions" style="display:none"></div>';this.append(G+s+J+t+"</div>");
K.loadActions(function(){g(w+" .sp-search-txt").val(K.q);K.search(K.q)})}else{if(typeof(y)=="string"){var B=this.data("sapnsGrid");
var w="#"+B.name;if(y=="loadData"){B.loadData()}else{if(y=="search"){var A="";if(typeof(x)=="boolean"){A=B.q
}else{if(x){A=x}}if(B.with_search){g(w+" .sp-search-txt").val(A)}B.search(A,true,v)
}else{if(y=="getSelectedIds"){return B.getSelectedIds()}else{if(y=="setRp"){B.rp=x;
g(w+" .sp-grid-rp [value="+x+"]").prop("selected",true)}else{if(y=="selectAll"){B.selectAll()
}else{if(y==="getQuery"){return{filters:B.filters,order:B.order}}}}}}}}}return this
}})(jQuery);