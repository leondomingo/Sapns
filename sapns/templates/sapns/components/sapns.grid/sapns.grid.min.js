...
if(h){j.push(new d({field:h[2],type:h[1]}))}}return j}function c(n){var j=this;if(typeof(n.caption)==="function"){n.caption=n.caption()
}if(typeof(n.actions)==="function"){n.actions=n.actions()}var h=[{id:"csv",title:"CSV",url:"{{tg.url('/dashboard/to_csv/')}}"},{id:"excel",title:"Excel",url:"{{tg.url('/dashboard/to_xls/')}}"}];
var m=n.pager_options;if(!m){m=[{val:10,desc:"10",sel:false},{val:50,desc:"50",sel:false},{val:100,desc:"100",sel:false},{val:0,desc:"{{_('All')}}",sel:false}]
}if(n.rp===undefined){n.rp=10}var l=g.inArray(n.rp*1,[10,50,100,0]);if(l===-1){m.push({val:n.rp*1,desc:n.rp,sel:true})
}else{m[l].sel=true}m.sort(function(i,p){if(i.val===0){return 1}else{if(p.val==0){return -1
}else{return i.val-p.val}}});var o=g.extend(true,{caption:"",name:"sapns_grid_"+Math.floor(Math.random()*999999),cls:"",with_search:true,with_filters:true,search_params:{},show_ids:false,link:"",q:"",user_filters:[],ch_attr:"",parent_id:"",cols:[],data:{},height:500,url_base:"",multiselect:false,allow_multiselect:true,actions:null,actions_inline:false,hide_check:false,hide_id:true,dblclick:null,select_first:false,shift_enabled:false,exportable:true,exportable_formats:h,with_pager:true,pag_n:1,rp:10,onLoad:null,resize:{min_width:60,after:null},default_:{col_width:60,col_align:"center",empty_value:""}},n);
g.extend(true,j,o);j.pager_options=m;j.this_page=0;j.total_count=0;j.total_pag=0;
j.ajx_data=JSON.stringify({});j.styles=[];j.setQuery(j.q);j._new_default={name:"new",url:'{{tg.url("/dashboard/new/")}}',require_id:"false",type:"new"};
...
}var p="#"+u.name;g(p+" .sp-col-title-sortable, "+p+" .sp-grid-cell-tip").resizable({handles:"e",minWidth:u.resize.min_width,resize:function(o,n){var j=n.element.width(),X=n.element.attr("col_num"),l=g(p+" .sp-grid-row").width(),i=g(p+" .sp-grid-cell[col_num="+X+"]").width();
g(p+" .sp-col-title-sortable[col_num="+X+"]").width(j);g(p+" .sp-grid-cell[col_num="+X+"]").width(j);
g(p+" .sp-grid-row").width(l+(j-i))},stop:function(l,j){var n=j.element.attr("col_num");
if(u.resize.after){u.resize.after(n*1)}else{var i=u.cols.length;if(u.cols[0].title==="id"){i-=1
}g.ajax({url:"{{tg.url('/dashboard/save_col_width/')}}",data:{cls:u.cls,length:i,col_num:n,width:j.element.width()},success:function(o){if(o.status){}}})
...
g(document).on("change",u,function(){var B=g(u).val();if(B==""){return}var F=r.exportable_formats;
var D="";if(typeof(r.exportable)==="object"){F=r.exportable.formats;if(r.exportable.data){for(k in r.exportable.data){var G=r.exportable.data[k];
if(typeof(G)==="function"){G=G()}D+='<input type="hidden" name="'+k+'" value="'+G+'">'
}}}var A="",I;for(var E=0,C=F.length;E<C;E++){if(F[E].id==B){I=F[E];break}}if(I.modal){g(this).find("option:first").prop("selected",true);
g.ajax({url:I.url,data:{cls:r.cls,q:r.query(),ch_attr:r.ch_attr,parent_id:r.parent_id},success:function(K){var J=false;
if(K.status){g("#sp-grid-export-dialog").remove();g('<div id="sp-grid-export-dialog" style="display:none"></div>').appendTo("body");
g("#sp-grid-export-dialog").html(K.content).dialog({title:"{{_('Export view to PDF')}}",modal:true,resizable:false,width:I.width||700,height:I.height||"auto",buttons:{"{{_('Ok')}}":function(){if(!J){J=true;
var L=[];g(".export-label.selected").each(function(){var O=g(this).attr("column-name");
L.push(O)});var M=g(".export-orientation:checked").attr("orientation");var N='<form action="'+I.export_url+'" method="get" target="_blank">                                                <input type="hidden" name="cls" value="'+r.cls+'">                                                <input type="hidden" name="q" value="'+r.query()+'">                                                <input type="hidden" name="ch_attr" value="'+r.ch_attr+'">                                                <input type="hidden" name="parent_id" value="'+r.parent_id+'">                                                <input type="hidden" name="orientation" value="'+M+'">                                                <input type="hidden" name="html" value="">                                                <input type="hidden" name="visible_columns" value=\''+JSON.stringify(L)+"'>"+D+"</form>";
g(N).appendTo("body").submit().remove();setTimeout(function(){J=false;g("#sp-grid-export-dialog").dialog("close")
},500)}},"{{_('Cancel')}}":function(){if(!J){g("#sp-grid-export-dialog").dialog("close")
}}}})}}})}else{var H='<form action="'+I.url+'" method="get">                    <input type="hidden" name="cls" value="'+r.cls+'">                    <input type="hidden" name="q" value="'+r.query()+'">                    <input type="hidden" name="ch_attr" value="'+r.ch_attr+'">                    <input type="hidden" name="parent_id" value="'+r.parent_id+'">'+D+"</form>";
g(H).appendTo("body").submit().remove();g(this).find("option:first").prop("selected",true)
...
}}else{U='<p class="sp-grid-no-filters">{{_("There are no filters")}}</p>'}g(y+" .sp-grid-filters").html(U)
}l();function i(q,R){g("<div/>").html(g(y+" .sp-grid-edit-filter").html()).dialog({title:"{{_('Edit filter')}}",modal:true,resizable:false,width:700,height:150,open:function(){if(q){g(this).find(".sp-grid-filter-field").val(q.field);
...
N.search(N.q)})}var L=y+" .sp-col-title-sortable";g(document).off("click",L).on("click",L,function(U){var W=g(this).attr("col_title");
