...
for(var R=0,q=N.user_filters.length;R<q;R++){var S=N.user_filters[R];if(S.name===U){N.setQuery(S.query);
...
g(n).val(S)}g("#sp-grid-save-filter-dialog").dialog("close")}else{if(V.msg){console.log(V.msg)
}i=false;alert("Error!")}},error:function(){i=false;alert("Error!")}})}},"{{_('Delete')}}":function(){if(!i){var S=g(".sp-grid-filter-name[grid-id="+y+"]").val();
