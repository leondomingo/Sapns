...
}e(this,"shift_enabled",false);e(this,"_new_default",{name:"new",url:"/dashboard/new/",require_id:"false",type:"new"});
...
var s=t.cols;if(typeof(s)=="function"){s=s()}var m=23;if(t.hide_check){m=-6}var g=s.length*42;
if(!t.hide_check){g+=23}if(!t.hide_id){g+=60}if(g<905){g=905}for(var B=0,x=s.length;
B<x;B++){var h=s[B];if(t.hide_id&&h.title=="id"){continue}m+=(h.width+5)}var C='<div class="sp-grid" style="width:'+(m+5)+'px;">';
var d='<div class="sp-grid-row" style="width:'+g+'px;">';if(!t.hide_check){d+='<div class="sp-col-title" style="width:23px;"><input class="sp-grid-select-all" type="checkbox"/></div>'
}if(t.actions_inline){d+='<div class="sp-col-title" style="%(actions_wd)s">*</div>'
}for(var B=0,x=s.length;B<x;B++){var h=s[B];var r=h.width;if(!r){r=t.default_.col_width
}if(t.hide_id&&h.title=="id"){continue}d+='<div class="sp-col-title" style="width:'+r+'px;">'+h.title+"</div>\n"
}d+="</div>";var F=t.data;if(typeof(F)=="function"){F=F()}var u=F.length;if(u>0){for(var B=0;
B<u;B++){var o=F[B];var f='<div class="sp-grid-row" style="width:'+g+'px;">\n';if(!t.hide_check){var A="";
if(B==u-1){A="border-radius: 0 0 0 5px;"}f+='<div class="sp-grid-cell" title="'+(B+1)+'" style="width:23px;'+A+'"><input class="sp-grid-rowid" type="checkbox" id_row="'+o[0]+'"></div>'
}if(t.actions_inline){var c="width: 100px;";if(!t.nonstd){c="width: 75px;"}var q='style="padding:2px;margin-left:5px;margin-right:5px;border:1px solid lightgray;"';
...
f+=sprintf(E,{actions_wd:c})+t.nonstd+"\n";f+="</div>\n"}if(B==0){C+=sprintf(d,{actions_wd:c})
}for(var z=0,p=s.length;z<p;z++){var h=s[z];var y=h.align;if(!y){y=t.default_.col_align
}var r=h.width;if(!r){r=t.default_.col_width}var e=o[z];if(!e){e=t.default_.empty_value
}if(t.hide_id&&h.title=="id"){continue}var w="";if(r>0){w=" width: "+r+"px;"}var A="";
if((t.hide_check&&B==u-1)&&(!t.hide_id&&z==0||t.hide_id&&z==1)){A="border-radius:0 0 0 5px;"
}if(B==u-1&&z==p-1){A="border-radius:0 0 5px 0;"}f+='<div class="sp-grid-cell" style="text-align:'+y+";"+w+A+'"';
if(e){f+='title="'+e.replace(/"/gi,"''")+'"'}else{f+='title="({{_("empty")}})"'}f+='clickable="true">'+e+"</div>\n"
}f+="</div>\n";C+=f}}else{var v=1;var D=m;if(t.actions_inline){v=3;D+=25}C+=sprintf(d,{actions_wd:c})+'<div class="sp-grid-row"><div class="sp-grid-cell sp-grid-noresults" title="{{_("No results")}}" style="width:'+D+'px;" >{{_("No results")}}</div></div>'
...
}b("#"+f.name+" .actions").html(f._loadActions(f.actions))}},f.actions);b.ajax(c)
}else{b("#"+f.name+" .actions").html(f._loadActions(f.actions))}}b("#"+f.name+" .sp-grid-cell").live("click",function(j){if(b(this).attr("clickable")=="true"){b("#"+f.name+" .sp-grid-select-all").attr("checked",false);
...
}j+="</form>\n";return j}function e(q,m,l,n,j){if(typeof(l.type)==="string"){if(j&&f.shift_enabled){if(l.type==="new"){l=f._new_default
}else{if(l.type==="edit"){l=f._edit_default}else{if(l.type==="delete"){l=f._delete_default
}else{if(l.type==="docs"){l=f._docs_default}}}}}var i=l.url;if(i[i.length-1]!="/"){i+="/"
}var o="";if(n){o="_blank"}if(l.type=="process"){if(l.require_id){if(q){i+=q}else{f.warningSelectedId();
return}}else{if(q){i+=q}}}else{if(l.type=="new"){i+=sprintf("%s/",f.cls)}else{if(l.type=="delete"){f.std_delete(m,l.url);
return}else{if(l.require_id){if(q){i+=sprintf("%s/%s",f.cls,q)}else{f.warningSelectedId();
return}}else{if(q){i+=sprintf("%s/%s",f.cls,q)}else{i+=sprintf("%s/",f.cls)}}}}}b(h(i,o)).appendTo("body").submit().remove()
}else{var p=m;if(l.require_id){if(p.length>0){l.type.f(p[0],p)}else{f.warningSelectedId();
return}}else{if(p.length>0){l.type.f(p[0],p)}else{l.type.f()}}}}function g(l,n,m,j){var i=f.getAction(n);
var o=l.parent().parent().find(".sp-grid-rowid").attr("id_row");e(o,[],i,m,j)}b("#"+f.name+" .new_inline").live("click",function(i){g(b(this),"new",i.ctrlKey||i.metaKey,i.shiftKey)
});if(f.actions_inline){b("#"+f.name+" .edit_inline").live("click",function(i){g(b(this),"edit",i.ctrlKey||i.metaKey,i.shiftKey)
...
j=[m]}var i=f.getAction("delete");f.std_delete(j,i.url)});b("#"+f.name+" .docs_inline").live("click",function(i){g(b(this),"docs",false,i.shiftKey)
...
var l=f.getSelectedIds();e(l[0],l,i,j.ctrlKey||j.metaKey,j.shiftKey)})}b("#grid-export_"+f.name).live("change",function(){var n=b("#grid-export_"+f.name+" option:selected").val();
