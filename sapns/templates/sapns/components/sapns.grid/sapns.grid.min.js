...
}}});function form(action,target){var came_from="";if(target!="_blank"){var came_from=self.url_base+"?q="+encodeURI(self.q).replace("-","%2D","g").replace('"',"%22","g").replace("+","%2B","g").replace("#","%23","g")+"&rp="+self.rp+"&pag_n="+self.pag_n+"&ch_attr="+self.ch_attr+"&parent_id="+self.parent_id
...
if(a[a.length-1]!="/"){a+="/"}var target="";console.log(ctrl);if(ctrl){target="_blank"
}if(act.type=="process"){a+=id}else{if(act.type=="new"){a+=sprintf("%s/",self.cls)
}else{a+=sprintf("%s/%s",self.cls,id)}}console.log(form(a,""));$(form(a,target)).appendTo("body").submit().remove()
}else{act.type.f(id)}}$("#"+self.name+" .new_inline").live("click",function(event){run_action($(this),"new",event.ctrlKey)
});$("#"+self.name+" .edit_inline").live("click",function(event){run_action($(this),"edit",event.ctrlKey)
...
}}if(self.with_search){$("#"+self.name+" .sp-search-txt").val(q)}self.search(q,true)
}else{if(arg1=="getSelectedIds"){return self.getSelectedIds()}}}}}return this}})(jQuery);