(function(g){function b(h){this.field=h.field.trim();this.operator=h.operator;this.value=h.value.trim();
this.active=h.active?h.active!==undefined:true}function a(h){h=h.toLowerCase().trim();
...
},stringify:function(){return sprintf('"%s" %s "%s"',this.field,this.operator,this.value)
}};function f(o){var q=o.split(",");var p=[];for(var n=0,j=q.length;n<j;n++){var h=q[n].trim().match('^"([^"]+)"\\s([a-z]{2,3})\\s"([^"]*)"$');
if(h){p.push(new b({field:h[1],operator:h[2],value:h[3]}))}}return p}function d(h){this.field=h.field;
if(h.type=="+"){this.type="asc"}else{if(h.type=="-"){this.type="desc"}else{this.type=h.type
}}}d.prototype={toString:function(){var h=this;var j;if(h.type=="asc"){j="+"}else{if(h.type=="desc"){j="-"
}}var i=a(h.field);return sprintf("%s%s",j,i)},stringify:function(){var h=this;var i;
if(h.type=="asc"){i="+"}else{if(h.type=="desc"){i="-"}}return sprintf('%s"%s"',i,h.field)
}};function e(p){var q=p.split(",");var j=[];for(var o=0,n=q.length;o<n;o++){var h=q[o].trim().match('^(\\+|\\-)"([^"]+)"$');
...
}else{h=t.filters.join(", ")}}if(t.order.length){if(h){h=h+", "+t.order.join(", ")
}else{h=t.order.join(", ")}}var n='<div style="padding:10px;font-size: 15px; '+sprintf('font-weight:bold;color:gray;height:%(hg)dpx;">{{_("Loading")}}...</div>',{hg:t.height-50});
...
g("#"+t.name+" .sp-grid-pager-desc").html(v);g("#"+t.name+" .sp-grid-current-page").val(t.pag_n);
...
if(h.filters.length){for(var m=0,j=h.filters.length;m<j;m++){n+=h.filters[m].tip()+"<br>"
}}else{n="No hay filtros"}return n}},position:{at:"top right",my:"bottom left"},style:"ui-tooltip-rounded"});
