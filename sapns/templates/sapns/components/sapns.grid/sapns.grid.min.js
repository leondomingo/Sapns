...
}if(self.hide_id&&col.title=="id"){continue}g_table+='<td class="sp-grid-cell" style="text-align: '+al+"; width: "+wd+'px;"';
...
}g_table+="</tr>"}}else{var n=1;if(self.actions_inline){n=3}g_table+='<tr class="sp-grid-row" style="width: 100%;"><td class="sp-grid-cell sp-grid-noresults" colspan="'+(cols.length+n)+'">{{_("No results")}}</td></tr>'
...
}}});if(self.dblclick){$("#"+self.name+" .sp-grid-cell").live("dblclick",function(event){if($(this).attr("clickable")=="true"){var row_id=$(this).parent().find(".sp-grid-rowid");
$("#"+self.name+" .sp-grid-rowid").each(function(){if($(this)!=row_id&&!self.multiselect&&!event.ctrlKey){$(this).attr("checked",false)
}});if(row_id.attr("checked")==true){row_id.attr("checked",false)}else{row_id.attr("checked",true)
}if(typeof(self.dblclick)=="function"){self.dblclick(row_id.attr("id_row")*1)}else{$("#"+self.name+"_"+self.dblclick).click()
