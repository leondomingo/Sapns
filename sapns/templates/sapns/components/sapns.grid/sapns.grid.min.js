...
g(this,"hide_check",false);g(this,"hide_id",true);g(this,"dblclick",null);g(this,"select_first",false);
...
}g(this,"shift_enabled",false);g(this,"exportable",true);var e=[{id:"csv",title:"CSV",url:"{{tg.url('/dashboard/tocsv')}}"},{id:"excel",title:"Excel",url:"{{tg.url('/dashboard/toxls')}}"}];
...
this.filters=[];this.order=[];this._new_default={name:"new",url:"/dashboard/new/",require_id:"false",type:"new"};
...
}b.prototype.getSelectedIds=function(){var e=this;var f=[];d("#"+e.name+" .sp-grid-row").each(function(){var g=d(this).find(".sp-grid-rowid");
...
if(h.attr("id_row")==g){f=h.parent();return}});return f};b.prototype.loadData=function(r){var B=this;
var z=B.cols;if(typeof(z)=="function"){z=z()}var t=23;if(B.hide_check){t=-6}var q=z.length*42;
if(!B.hide_check){q+=23}if(!B.hide_id){q+=60}var f=905;if(q<f){q=f}var x="";for(var L=0,H=z.length;
L<H;L++){var s=z[L];if(s.title!="id"){x+='<option value="'+s.title+'">'+s.title+"</option>"
}if(B.hide_id&&s.title=="id"){continue}t+=(s.width+5)}d("#"+B.name+" .sp-grid-filter-field").html(x);
var M='<div class="sp-grid" style="width:'+(t+5)+'px;">';var g='<div class="sp-grid-row" style="width:'+q+'px;">';
if(!B.hide_check){g+='<div class="sp-col-title" style="width:23px;"><input class="sp-grid-select-all" type="checkbox"></div>'
}if(B.actions_inline){g+='<div class="sp-col-title" style="%(actions_wd)s">*</div>'
}var C={};for(var L=0,H=B.order.length;L<H;L++){var E=B.order[L];E.index=L+1;C[B.order[L].field]=E
}for(var L=0,H=z.length;L<H;L++){var s=z[L];var y=s.width;if(!y){y=B.default_.col_width
}if(B.hide_id&&s.title=="id"){continue}var p="";var P="";var A="";if(C[s.title]){var E=C[s.title];
p="sp-grid-ordered-col";P=' order_type="'+E.type+'"';var v;if(E.type==="asc"){v="+"
}else{v="-"}A=" ("+E.index+v+")"}g+='<div class="sp-col-title sp-col-title-sortable '+p+'"'+P+' style="width:'+y+'px;" col_title="'+s.title+'">'+s.title+A+"</div>"
}g+="</div>";var Q=B.data;if(typeof(Q)=="function"){Q=Q()}var D=Q.length;if(D>0){for(var L=0;
L<D;L++){var u=Q[L];var m='<div class="sp-grid-row" style="width:'+q+'px;">';if(!B.hide_check){var K="";
if(L==D-1){K="border-radius:0 0 0 5px;"}m+='<div class="sp-grid-cell" title="'+u[0]+'" style="width:23px;'+K+'"><input class="sp-grid-rowid" type="checkbox" id_row="'+u[0]+'"></div>'
}if(B.actions_inline){var e="width:107px;";if(!B.nonstd){e="width:82px;"}var O='<div class="sp-grid-cell" style="%(actions_wd)s"><img class="inline_action edit_inline" title="{{_("Edit")}}" src="{{tg.url("/images/sapns/icons/edit.png")}}"><img class="inline_action delete_inline" title="{{_("Delete")}}" src="{{tg.url("/images/sapns/icons/delete.png")}}"><img class="inline_action docs_inline" title="{{_("Docs")}}" src="{{tg.url("/images/sapns/icons/docs.png")}}">';
m+=sprintf(O,{actions_wd:e})+B.nonstd+"</div>"}if(L==0){M+=sprintf(g,{actions_wd:e})
}for(var J=0,w=z.length;J<w;J++){var s=z[J];var I=s.align;if(!I){I=B.default_.col_align
}var y=s.width;if(!y){y=B.default_.col_width}var h=u[J];if(!h){h=B.default_.empty_value
}if(B.hide_id&&s.title=="id"){continue}var G="";if(y>0){G=" width:"+y+"px;"}var K="";
if((B.hide_check&&L==D-1)&&(!B.hide_id&&J==0||B.hide_id&&J==1)){K="border-radius:0 0 0 5px;"
}if(L==D-1&&J==w-1){K="border-radius:0 0 5px 0;"}m+='<div class="sp-grid-cell sp-grid-cell-tip" style="text-align:'+I+";"+G+K+'"';
if(h){m+=sprintf('title="%s"',h.replace(/"/gi,"''"))}else{m+='title="({{_("empty")}})"'
}m+='clickable="true">'+h+"</div>"}m+="</div>";M+=m}}else{var F=1;var N=t;if(B.actions_inline){F=3;
N+=25}M+=sprintf(g,{actions_wd:e})+'<div class="sp-grid-row"><div class="sp-grid-cell sp-grid-noresults" title="{{_("No results")}}" style="width:'+N+'px;" >{{_("No results")}}</div></div>'
}M+="</div>";d("#"+B.name).find(".sp-grid-parent").html(M);d(".sp-col-title-sortable").disableSelection();
if(B.select_first&&B.with_search&&B.q&&D<5&&D>0){d("#"+B.name+" .sp-grid-rowid:first").attr("checked",true)
...
}m+="</form>";return m}function g(s,o,n,p,m){if(typeof(n.type)==="string"){if(m&&h.shift_enabled){if(n.type==="new"){n=h._new_default
...
var q=o.rel_tables[n].attr_title;p+='<li><span style="font-weight:bold;">'+r+'</span> (<span style="color:#777;">'+q+"</span>)</li>"
