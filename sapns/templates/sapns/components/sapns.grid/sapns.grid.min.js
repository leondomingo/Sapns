...
}else{if(S.type==="delete"){r.std_delete(I,S.url);return}else{if(S.require_id){if(H){V+=sprintf("%s/%s",r.cls,H)
}else{r.warningSelectedId();return}}else{if(H){V+=sprintf("%s/%s",r.cls,H)}else{V+=sprintf("%s/",r.cls)
}}}}}g(h(V,W)).appendTo("body").submit().remove()}else{var X=JSON.parse(S.data),O=false;
var T={};T[X.param_name]=H;T[X.param_name+"$"]=JSON.stringify(I);var C="{{_('Ok')}}";
if(X.button_title){C=X.button_title}var A=true;if(X.refresh!==undefined){A=X.refresh
