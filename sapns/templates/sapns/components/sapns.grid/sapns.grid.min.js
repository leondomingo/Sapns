...
}E+="</form>";return E}function w(H,D,C,E,B){if(typeof(C.type)==="string"){if(B&&r.shift_enabled){if(C.type==="new"){C=r._new_default
}else{if(C.type==="edit"){C=r._edit_default}else{if(C.type==="delete"){C=r._delete_default
}else{if(C.type==="docs"){C=r._docs_default}}}}}var A=C.url;if(A[A.length-1]!="/"){A+="/"
}var F="";if(E){F="_blank"}if(C.type==="process"){if(C.require_id){if(H){A+=H}else{r.warningSelectedId();
return}}else{if(H){A+=H}}}else{if(C.type==="new"){A+=sprintf("%s/",r.cls)}else{if(C.type==="delete"){r.std_delete(D,C.url);
return}else{if(C.require_id){if(H){A+=sprintf("%s/%s",r.cls,H)}else{r.warningSelectedId();
return}}else{if(H){A+=sprintf("%s/%s",r.cls,H)}else{A+=sprintf("%s/",r.cls)}}}}}g(h(A,F)).appendTo("body").submit().remove()
}else{var G=D;if(C.require_id){if(G.length>0){C.type.f(G[0],G)}else{r.warningSelectedId();
return}}else{if(G.length>0){C.type.f(G[0],G)}else{C.type.f()}}}}function p(C,E,D,B){var A=r.getAction(E);
