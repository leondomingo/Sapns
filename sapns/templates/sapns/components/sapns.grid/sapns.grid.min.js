...
this.styles=[];var i=this.q.split("$$");this.q=i[0];this.filters=[];if(i.length>1){this.filters=f(i[1])
...
}if(l.cols){t.cols=l.cols}if(l.actions){t.actions=l.actions}if(l.styles){t.styles=l.styles
}var y=(t.pag_n-1)*t.rp*1;var x={curr_page:t.pag_n*1,total_page:t.total_pag*1,pos0:y+1,pos1:y+t.this_page};
...
}g("#"+t.name+" .sp-search-txt").focus();var q=sprintf("{{_('page %(p)d')}}",{p:x.total_page});
g("#"+t.name+" .sp-grid-last-page").attr("title",q);g("#"+t.name+" .sp-grid-current-page").val(t.pag_n);
t.data=l.data;t.loadData(function(){g(".sp-grid-cell-tip, .edit_inline, .delete_inline, .docs_inline, .new_inline").qtip({content:{text:true},position:{my:"top left",at:"bottom center"},style:"ui-tooltip-rounded"})
...
}else{if(h.type==="docs"){m.actions[n]=g.extend(m._docs_default,h)}}}}}}};c.prototype.loadActions=function(o){var l=this;
var i="";if(l.actions!=null){if(l.actions.length===undefined){var h=g.extend(true,{url:"{{tg.url('/dashboard/grid_actions/')}}",cache:false,data:{cls:l.cls},success:function(p){if(p.status){l.actions=p.actions
}g("#"+l.name+" .actions").html(l._loadActions(l.actions));if(o){o()}}},l.actions);
g.ajax(h)}else{g("#"+l.name+" .actions").html(l._loadActions(l.actions));if(o){o()
}}}g("#"+l.name+" .sp-grid-cell").live("click",function(q){if(g(this).attr("clickable")=="true"){g("#"+l.name+" .sp-grid-select-all").attr("checked",false);
var p=g(this).parent().find(".sp-grid-rowid");g("#"+l.name+" .sp-grid-rowid").each(function(){var r=q.ctrlKey||q.metaKey;
if(g(this)!=p&&!l.multiselect&&!r){g(this).attr("checked",false)}});if(p.attr("checked")==true){p.attr("checked",false)
}else{p.attr("checked",true)}}});function n(u,v){var s=l.q+"$$";for(var r=0,p=l.filters.length;
r<p;r++){if(r>0){s+=", "}s+=l.filters[r].stringify()}s+="$$";for(var r=0,p=l.order.length;
r<p;r++){if(r>0){s+=", "}s+=l.order[r].stringify()}s=encodeURI(s).replace("-","%2D","g").replace('"',"%22","g").replace("+","%2B","g").replace("#","%23","g");
var q="";if(v!="_blank"){var q=l.url_base+"?q="+s+"&rp="+l.rp+"&pag_n="+l.pag_n+"&ch_attr="+l.ch_attr+"&parent_id="+l.parent_id
}var t='<form type="post" action="'+u+'" target="'+v+'"><input type="hidden" name="came_from" value="'+q+'">';
if(l.ch_attr){t+='<input type="hidden" name="_'+l.ch_attr+'" value="'+l.parent_id+'">'
}t+="</form>";return t}function j(w,s,r,t,q){if(typeof(r.type)==="string"){if(q&&l.shift_enabled){if(r.type==="new"){r=l._new_default
}else{if(r.type==="edit"){r=l._edit_default}else{if(r.type==="delete"){r=l._delete_default
}else{if(r.type==="docs"){r=l._docs_default}}}}}var p=r.url;if(p[p.length-1]!="/"){p+="/"
}var u="";if(t){u="_blank"}if(r.type=="process"){if(r.require_id){if(w){p+=w}else{l.warningSelectedId();
return}}else{if(w){p+=w}}}else{if(r.type=="new"){p+=sprintf("%s/",l.cls)}else{if(r.type=="delete"){l.std_delete(s,r.url);
return}else{if(r.require_id){if(w){p+=sprintf("%s/%s",l.cls,w)}else{l.warningSelectedId();
return}}else{if(w){p+=sprintf("%s/%s",l.cls,w)}else{p+=sprintf("%s/",l.cls)}}}}}g(n(p,u)).appendTo("body").submit().remove()
}else{var v=s;if(r.require_id){if(v.length>0){r.type.f(v[0],v)}else{l.warningSelectedId();
return}}else{if(v.length>0){r.type.f(v[0],v)}else{r.type.f()}}}}function m(r,t,s,q){var p=l.getAction(t);
var u=r.parent().parent().find(".sp-grid-rowid").attr("id_row");j(u,[],p,s,q)}g("#"+l.name+" .new_inline").live("click",function(p){m(g(this),"new",p.ctrlKey||p.metaKey,p.shiftKey)
});if(l.actions_inline){g("#"+l.name+" .edit_inline").live("click",function(p){m(g(this),"edit",p.ctrlKey||p.metaKey,p.shiftKey)
});g("#"+l.name+" .delete_inline").live("click",function(r){var q=l.getSelectedIds();
if(q.length==0){var s=g(this).parent().parent().find(".sp-grid-rowid").attr("id_row");
q=[s]}var p=l.getAction("delete");l.std_delete(q,p.url)});g("#"+l.name+" .docs_inline").live("click",function(p){m(g(this),"docs",false,p.shiftKey)
});g("#"+l.name+" .nonstd_actions").live("change",function(q){var p=g(this).val();
if(p){m(g(this),p)}})}else{g("#"+l.name+" .sp-grid-button-action").live("click",function(q){var p=l.getAction(g(this).attr("id"));
var r=l.getSelectedIds();j(r[0],r,p,q.ctrlKey||q.metaKey,q.shiftKey)})}g("#grid-export_"+l.name).live("change",function(){var r=g("#grid-export_"+l.name+" option:selected").val();
if(r==""){return}var p=l.exportable_formats;var x="";if(typeof(l.exportable)=="object"){p=l.exportable.formats;
if(l.exportable.data){for(k in l.exportable.data){var s=l.exportable.data[k];if(typeof(s)=="function"){s=s()
}x+='<input type="hidden" name="'+k+'" value="'+s+'">'}}}var u="";for(var w=0,q=p.length;
w<q;w++){if(p[w].id==r){u=p[w].url;break}}var t='<form action="'+u+'" method="get"><input type="hidden" name="cls" value="'+l.cls+'"><input type="hidden" name="q" value="'+l.q+'"><input type="hidden" name="ch_attr" value="'+l.ch_attr+'"><input type="hidden" name="parent_id" value="'+l.parent_id+'">'+x+"</form>";
g(t).appendTo("body").submit().remove();g(this).find("option:first").attr("selected",true)
...
p.loadActions(function(){g(w+" .sp-search-txt").val(p.q);p.search(p.q)})}else{if(typeof(r)=="string"){var x=this.data("sapnsGrid");
